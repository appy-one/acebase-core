function c(input: Uint8Array | number[], length: number, result: string[]) {
    const b = [0, 0, 0, 0, 0];
...
function encode(arr: Uint8Array|number[]) {
    ...
    const input = arr, result: string[] = [], remainder = input.length % 4, length = input.length - remainder;
...
export const ascii85 = {
    ...
    decode: function(input: string) {
        ...
        let t: number, x: number, y: number, d: number;
        for(let i = 0; i < n; ++i) {
            if(input.charAt(i) == 'z'){
                r.push(0, 0, 0, 0);
                continue;
