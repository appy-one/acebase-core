...
const
    ourEpoch = 1529539200000, // use own epoch instead of standard Unix? Allows cuids far into 2130 to be lexicographically sortable // new Date('2018-06-21').getTime()
    dictionary = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz',
    base = dictionary.length, // 62
    counterBlockSize = 3,
    maxCounterNr = Math.pow(base, counterBlockSize),
    randomBlockSize = 6,
    maxRandomNr = Math.pow(base, randomBlockSize);
...
function encode(nr: number) {
    if (nr === 0) { return dictionary[0]; }
...
export default function cuid (timebias = 0) {
    ...
    const hires = performance.timeOrigin + performance.now() + timebias - ourEpoch,
        rational = Math.floor(hires),
        fraction = Math.round((hires - rational) * 1_000_000); // Use 6 decimals to get nanoseconds
