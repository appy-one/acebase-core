import { serialize, deserialize, serialize2, deserialize2, detectSerializeVersion } from './transport';
...
describe('Transport (de)serializing', () => {
    ...
    it('single values', () => {
        ...
        }
        {
            ...
            expect(ser as any).toEqual({ '.type': 'date', '.val': val.toISOString() });
        ...
        }
        {
            ...
            const val = /test/ig;
            const ser = serialize2(val);
            expect(ser as any).toEqual({ '.type': 'regexp', '.val': `/${val.source}/${val.flags}` });
            const check = deserialize2(ser);
            expect(check).toEqual(val);
            const ver = detectSerializeVersion(ser);
            expect(ver).toBe(2);
        ...
        }
        {
            ...
            const val = encodeString('AceBase rocks').buffer;
            const ser = serialize2(val);
            expect(ser as any).toEqual({ '.type': 'binary', '.val': `<~6"=Im@<6!&Ec5H'Er~>` });
            const check = deserialize2(ser);
            expect(check).toEqual(val);
            const ver = detectSerializeVersion(ser);
            expect(ver).toBe(2);
        ...
        }
        {
            ...
            const val = new PathReference('other/path');
            const ser = serialize2(val);
            expect(ser as any).toEqual({ '.type': 'reference', '.val': `other/path` });
            const check = deserialize2(ser);
            expect(check).toEqual(val);
            const ver = detectSerializeVersion(ser);
            expect(ver).toBe(2);
        ...
        }
        {
            ...
            const val = BigInt(str);
            const ser = serialize2(val);
            expect(ser as any).toEqual({ '.type': 'bigint', '.val': str });
            const check = deserialize2(ser);
            expect(check).toEqual(val);
            const ver = detectSerializeVersion(ser);
            expect(ver).toBe(2);
    ...
    it('object values', () => {
        ...
        }
        {
            ...
            const val = { text: 'Some text', date: new Date() };
            const ser = serialize2(val);
            expect(ser as any).toEqual({ text: val.text, date: { '.type': 'date', '.val': val.date.toISOString() } });
            const check = deserialize2(ser);
            expect(check).toEqual(val);
            const ver = detectSerializeVersion(ser);
            expect(ver).toBe(2);
